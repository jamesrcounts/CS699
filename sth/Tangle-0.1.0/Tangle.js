...
var Tangle = this.Tangle = function (rootElement, modelClass) {
    ...
    function getOptionsForElement(element) {   // might use dataset someday
        ...
        for (var i = 0, length = attributes.length; i < length; i++) {
            ...
            if (!attrName || !regexp.test(attrName)) {
                continue;
            }
    ...
    function getFormatterForFormat(formatAttribute, varNames) {
        ...
        if (!formatter) {
            formatter = getFormatterForSprintfFormat(formatAttribute, varNames);
        }
        if (!formatter) {
            log("Tangle: unknown format: " + formatAttribute);
            formatter = getFormatterForFormat(null, varNames);
        }
    ...
    function getFormatterForSprintfFormat(formatAttribute, varNames) {
        if (!sprintf || !formatAttribute.test(/\%/)) {
            return null;
        }
    ...
    function addSetterForVariables(setter, varNames) {
        ...
        for (var i = 0; i < varNames.length; i++) {
            ...
            if (!_setterInfosByVariableName[varName]) {
                _setterInfosByVariableName[varName] = [];
            }
    ...
    function applySettersForVariables(varNames) {
        ...
        for (var i = 0, ilength = varNames.length; i < ilength; i++) {
            ...
            for (var j = 0, jlength = setterInfos.length; j < jlength; j++) {
                ...
                if (setterInfo.setterID in appliedSetterIDs) {
                    continue;
                }  // if we've already applied this setter, move on
    ...
    function getValue(varName) {
        var value = _model[varName];
        if (value === undefined) {
            log("Tangle: unknown variable: " + varName);
            return 0;
        }
        return value;
    }
    ...
    function setValues(obj) {
        ...
        for (var varName in obj) {
            ...
            if (oldValue === undefined) {
                log("Tangle: setting unknown variable: " + varName);
                continue;
            }
            if (oldValue === value) {
                continue;
            }  // don't update if new value is the same
    ...
    function updateModel(shouldInitialize) {
        ...
        if (shouldInitialize) {
            shadowModel.initialize();
        }
        shadowModel.update();

        var changedVarNames = [];
        for (var varName in shadowModel) {
            if (!shadowModel.hasOwnProperty(varName)) {
                continue;
            }
...
Tangle.formats["default"] = function (value) {
    return "" + value;
};
